...
A_minus = speye(n^2) - (tau / 2)^2 * A; 
A_plus  = speye(n^2) + (tau / 2)^2 * A;
...
for t = T_mesh
    fg    = f(t + tau, X, Y) + f(t, X, Y);

    left  = arrayfun(@(y) g([-1; 0], t, 0, y) + g([-1; 0], t + tau, 0, y), Y(:, 1));
    right = arrayfun(@(y) g([ 1; 0], t, 1, y) + g([ 1; 0], t + tau, 1, y), Y(:, 1));
    top   = arrayfun(@(x) g([ 0; 1], t, x, 1) + g([ 0; 1], t + tau, x, 1), X(1, :));
    bot   = arrayfun(@(x) g([ 0;-1], t, x, 0) + g([ 0;-1], t + tau, x, 0), X(1, :));
